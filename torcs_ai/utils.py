...
def start_torcs_server() -> bool:
    ...
    logger.info(
        "   set SDL_VIDEODRIVER=windib && wtorcs.exe -r config\\raceman\\quickrace.xml"
    )
...
def analyze_ml_models() -> None:
    ...
    try:
        ...
        for _ in range(n_test):
            ...
            test_targets["steer"].append(
                ml_racing_ai.expert_steering(speed, angle, track_pos, curvature)
            )
            test_targets["accel"].append(
                ml_racing_ai.expert_acceleration(speed, angle, curvature)
            )
            test_targets["brake"].append(
                ml_racing_ai.expert_braking(speed, angle, curvature)
            )
        ...
        for i, action in enumerate(["steer", "accel", "brake"]):
            if hasattr(ml_racing_ai, "models") and action in ml_racing_ai.models:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    axes[0, i].barh(
                        range(len(feature_names)), model.feature_importances_
                    )
                ...
                axes[1, i].hist(
                    errors, bins=30, alpha=0.7, color=["blue", "green", "red"][i]
                )
...
def generate_racing_insights() -> None:
    ...
    logger.info(
        f"🏎️ Speed: Avg={avg_speed:.1f}, Max={max_speed:.1f}, Std={speed_std:.1f}"
    )
    ...
    on_track_percentage = (
        sum(1 for pos in track_positions if pos <= 1.0) / len(track_positions) * 100
    )

    logger.info(
        f"🎯 Track Position: Avg Error={avg_track_error:.3f}, Max Error={max_track_error:.3f}, On Track={on_track_percentage:.1f}%"
    )
    ...
    if len(rewards) > 10:
        ...
        logger.info(
            f"💰 Rewards: Avg={avg_reward:.3f}, Trend={reward_trend:+.4f} {trend_status}"
        )
    else:
    ...
    logger.info(
        f"🎮 Controls: Steer={avg_steer:.3f}, Accel={avg_accel:.3f}, Brake={avg_brake:.3f}"
    )
    ...
    if ml_racing_ai.is_trained:
        ...
        logger.info(
            f"   Training Data: {len(ml_racing_ai.data_collector.experiences)} experiences"
        )
...
def calculate_adaptive_exploration(scenarios: dict) -> float:
    ...
    if (
        scenarios["emergency"]
        or scenarios["wall_proximity"]
        or scenarios["spin_recovery"]
    ):
...
def apply_scenario_adaptations(
    sensor_data: dict, actions: dict, scenarios: dict
) -> None:
...
def apply_competitive_strategy(
    actions: dict, strategy: dict, sensor_data: dict
) -> None:
