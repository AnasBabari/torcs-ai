...
class ServerState:
    ...
    def _format_sensor(self, k: str) -> str:
        ...
        if k == "gear":
            ...
            if gear_label == "0":
                gear_label = "N"
            return gs[:p] + "(%s)" % gear_label + gs[p + 3 :]
        elif k == "damage":
            ...
            return "%6.3f %s" % (value, bargraph(value, 0.3, 0.5, 50, "z"))
        elif k == "trackPos":
            cx = "<" if value >= 0 else ">"
            ...
            if frontwheelradpersec:
                skid = (
                    0.5555555555 * self.d.get("speedX", 0) / frontwheelradpersec
                    - 0.66124
                )
            return bargraph(skid, -0.05, 0.4, 50, "*")
        elif k == "slip":
...
class DriverAction:
    ...
    def clip_to_limits(self) -> None:
        ...
        if self.d["meta"] not in [0, 1]:
            self.d["meta"] = 0
        if (
            not isinstance(self.d["focus"], list)
...
class Client:
    ...
    def setup_connection(self) -> None:
        ...
        for attempt in range(1, self.connect_attempts + 1):
            ...
            try:
                raw_bytes, addr = self.so.recvfrom(DATA_SIZE)
                ...
                raise ConnectionError(
                    f"Error receiving server identification: {e}"
                ) from e
    ...
    def get_servers_input(self) -> None:
        ...
        while True:
            try:
                ...
                raise TimeoutError(
                    f"Timed out waiting for TORCS on port {self.port}"
                ) from exc
            ...
            if "***identified***" in text:
                ...
                logger.info(
                    f"Server stopped race on {self.port}. Position: {self.S.d.get('racePos', 'unknown')}"
                )
                self.shutdown()
                return
